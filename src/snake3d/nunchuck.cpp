...
inline bool Nunchuck::Connect() { return _i2c->connect(SLAVE_ADDRESS); }
...
const Data *Nunchuck::data() {
  ...
  // uint16_t
  _data.accelerometer.x = static_cast<uint16_t>(
      _raw_data[ACCELEROMETER_X] << 2 | (_raw_data[MIXED] & 0b00001100));
  _data.accelerometer.y = static_cast<uint16_t>(
      _raw_data[ACCELEROMETER_Y] << 2 | (_raw_data[MIXED] & 0b00110000));
  _data.accelerometer.z = static_cast<uint16_t>(
      _raw_data[ACCELEROMETER_Z] << 2 | (_raw_data[MIXED] & 0b11000000));
  _data.z_button =
      static_cast<ButtonState>(static_cast<bool>(_raw_data[MIXED] & 0b1));
  _data.c_button =
      static_cast<ButtonState>(static_cast<bool>(_raw_data[MIXED] & 0b10));
  return &_data;
}
